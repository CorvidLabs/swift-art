...
public struct RandomSource: Sendable {
    ...
    /// Generates a random Point2D within the specified bounds.
    public mutating func nextPoint2D(
    ...
    /// Generates a random Point3D within the specified bounds.
    public mutating func nextPoint3D(
        xRange: ClosedRange<Double> = 0...1,
        yRange: ClosedRange<Double> = 0...1,
    ...
    public mutating func nextAngle() -> Double {
        nextDouble() * 2 * .pi
    }

    /// Generates a random color with optional constraints.
    public mutating func nextColor(
        alphaRange: ClosedRange<Double> = 1...1
    ) -> RGBColor {
        RGBColor(
            red: nextDouble(),
            green: nextDouble(),
            blue: nextDouble(),
            alpha: nextDouble(in: alphaRange)
        )
    }
}
